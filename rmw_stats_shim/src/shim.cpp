...
{
  auto ret = reinterpret_cast<decltype(rmw_publish_loaned_message) *>(impl)(publisher, ros_message, allocation);
  if (ret == RMW_RET_OK) {
    StatCollector::instance().onPublish(publisher);
  }
  return ret;
...
{
  auto ret =
    reinterpret_cast<decltype(rmw_publish_serialized_message) *>(impl)(publisher, serialized_message, allocation);
  if (ret == RMW_RET_OK) {
    StatCollector::instance().onPublish(publisher);
  }
  return ret;
...
{
  auto ret = reinterpret_cast<decltype(rmw_take) *>(impl)(subscription, ros_message, taken, allocation);
...
{
  auto ret =
    reinterpret_cast<decltype(rmw_take_with_info) *>(impl)(subscription, ros_message, taken, message_info, allocation);
  if (ret == RMW_RET_OK && *taken) {
...
{
  auto ret =
    reinterpret_cast<decltype(rmw_take_loaned_message) *>(impl)(subscription, loaned_message, taken, allocation);
  if (ret == RMW_RET_OK && *taken) {
    StatCollector::instance().onReceive(subscription);
  }
  return ret;
