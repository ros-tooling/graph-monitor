...
/// Names come from MockedNode, which is enough to see the monitor carry them into the graph
/// message. A node listed in `unreachable` has no parameter services up yet.
...
TEST_F(GraphMonitorTest, parameter_observation_limits_come_from_the_configuration)
{
  ...
  EXPECT_EQ(blocked->started_count(), 2u) << "the configured cap should bound concurrent observations";
  monitor.reset();
}
