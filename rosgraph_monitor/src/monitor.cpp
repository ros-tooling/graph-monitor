...
std::size_t
std::hash<RosRmwGid>::operator()(const RosRmwGid &id) const noexcept {
  constexpr std::size_t u64s = sizeof(uint64_t);
  static_assert(sizeof(RosRmwGid) == (3 * u64s) ||
                sizeof(RosRmwGid) == (2 * u64s));
...
bool match_any_prefixes(const std::vector<std::string> &prefixes,
...
std::string gid_to_str(const RosRmwGid &gid) { return gid_to_str(&gid[0]); }
...
RosGraphMonitor::EndpointTracking::EndpointTracking(
    ...
    : topic_name(topic_name),
      node_name(info.node_namespace() == "/"
                    ...
                    : info.node_namespace() + "/" + info.node_name()),
      info(info), last_stats_timestamp(now) {}
...
RosGraphMonitor::RosGraphMonitor(
    ...
    : config_(config), now_fn_(now_fn), node_graph_(node_graph),
      logger_(logger), graph_change_event_(node_graph->get_graph_event()) {
  update_graph();
  watch_thread_ =
      std::thread(std::bind(&RosGraphMonitor::watch_for_updates, this));
}

RosGraphMonitor::~RosGraphMonitor() {
  shutdown_.store(true);
  // Set the predicate and notify the condition_variable to wake up the watch
  // thread immediately
...
std::optional<RosGraphMonitor::EndpointTrackingMap::iterator>
RosGraphMonitor::add_publisher(const std::string &topic_name,
  ...
  auto [it, inserted] =
      publishers_.emplace(info.endpoint_gid(), proposed_tracking);
  auto &[gid, tracking] = *it;
  publisher_lookup_.insert_or_assign(
      std::make_pair(tracking.node_name, tracking.topic_name), gid);
  if (inserted) {
    RCLCPP_DEBUG(logger_, "New Publisher: %s::%s (%s)",
...
RosGraphMonitor::lookup_publisher(const std::string &node_name,
...
std::optional<RosGraphMonitor::EndpointTrackingMap::iterator>
RosGraphMonitor::add_subscription(const std::string &topic_name,
                                  const rclcpp::TopicEndpointInfo &info) {
  EndpointTracking proposed_tracking(topic_name, info, now_fn_());
  if (ignore_node(proposed_tracking.node_name)) {
    return std::nullopt;
  ...
  auto [it, inserted] =
      subscriptions_.emplace(info.endpoint_gid(), proposed_tracking);
  auto &[gid, tracking] = *it;
  subscription_lookup_.insert_or_assign(
      std::make_pair(tracking.node_name, tracking.topic_name), gid);
  if (inserted) {
    RCLCPP_DEBUG(logger_, "New Subscription: %s::%s (%s)",
                 tracking.node_name.c_str(), tracking.topic_name.c_str(),
                 gid_to_str(tracking.info.endpoint_gid()).c_str());
  }
  return it;
...
std::optional<RosRmwGid>
RosGraphMonitor::lookup_subscription(const std::string &node_name,
                                     const std::string &topic_name) const {
  try {
...
bool RosGraphMonitor::topic_period_ok(
  ...
  const std::chrono::nanoseconds chrono_deadline =
  ...
  const auto allowed_error =
      chrono_deadline * config_.topic_statistics.deadline_allowed_error;
  return period_error <= allowed_error;
}

void RosGraphMonitor::track_endpoint_updates(
    const TopicsToTypes &observed_topics_and_types) {
  // Mark all stale as base state
  ...
  for (const auto &[topic_name, topic_types] : observed_topics_and_types) {
    // Assumption: "multiple types on the topic" is an error already handled
    // elsewhere
    bool count_topic =
    ...
    for (const auto &endpoint_info :
         node_graph_->get_publishers_info_by_topic(topic_name)) {
    ...
    for (const auto &endpoint_info :
         node_graph_->get_subscriptions_info_by_topic(topic_name)) {
      ...
      bool count_subs_from_node =
          config_.continuity.ignore_subscriber_nodes.count(
              tracking.node_name) == 0;
  ...
  // For now just delete them, there isn't a meaningful health case to track at
  // this point Also remove endpoints from missing node, that node missing is
  // the important error.
  ...
  if (config_.continuity.enable) {
    for (auto it = topic_endpoint_counts_.begin();
         it != topic_endpoint_counts_.end();) {
      ...
      // Delete any lingering tracking with no matches at all, the topic doesn't
      // exist anymore
      if (counts.pubs == 0 && counts.subs == 0) {
...
std::unique_ptr<diagnostic_msgs::msg::DiagnosticArray>
RosGraphMonitor::evaluate() {
  auto now = now_fn_();
  ...
  for (const auto &[node_name, node_info] : nodes_) {
    if (node_info.missing) {
      if (match_any_prefixes(config_.nodes.warn_only_prefixes, node_name)) {
        msg->status.push_back(statusMsg(DiagnosticStatus::WARN,
                                        "Optional node missing: " + node_name,
                                        "Node::" + node_name));
      } else {
        msg->status.push_back(statusMsg(DiagnosticStatus::ERROR,
  ...
  for (const std::string &node_name : returned_nodes_) {
    msg->status.push_back(statusMsg(
        DiagnosticStatus::OK, "Node OK: " + node_name, "Node::" + node_name));
  }
  ...
  for (const auto &[gid, tracking] : publishers_) {
    ...
    bool stale = (now - tracking.last_stats_timestamp) >
                 config_.topic_statistics.stale_timeout;
    if (deadline_not_set(deadline)) {
      // No deadline, don't care
    } else if (stale) {
      // Haven't received topic statistics recently enough, likely this means
      // it's not running
      msg->status.push_back(statusMsg(
          DiagnosticStatus::ERROR,
          "Stale topic statistics for publisher with deadline", subname));
      ...
      msg->status.push_back(statusMsg(
          ...
          "Publisher sending slower than promised deadline", subname));
    } else {
      msg->status.push_back(statusMsg(DiagnosticStatus::OK,
  ...
  for (const auto &[gid, tracking] : subscriptions_) {
    ...
    if (deadline_not_set(deadline)) {
      ...
      msg->status.push_back(statusMsg(
          DiagnosticStatus::ERROR,
          "Stale topic statistics for subscription with deadline", subname));
    } else if (!tracking.period_stat.has_value()) {
      // Not stale yet, but also not yet received statistics info. Just waiting.
    } else if (!topic_period_ok(*tracking.period_stat, deadline)) {
      // Have received topic stats and it isn't in good range
      msg->status.push_back(statusMsg(
          DiagnosticStatus::WARN,
          "Subscription receiving slower than requested deadline", subname));
    } else {
      msg->status.push_back(
          statusMsg(DiagnosticStatus::OK,
                    "Subscription receive frequency acceptable", subname));
    }
  }
...
GraphMonitorConfiguration &RosGraphMonitor::config() { return config_; }

const GraphMonitorConfiguration &RosGraphMonitor::config() const {
  return config_;
}

void RosGraphMonitor::on_topic_statistics(
    const rosgraph_monitor_msgs::msg::TopicStatistics &msg) {
  for (const auto &stat : msg.statistics) {
    std::optional<RosRmwGid> maybe_gid;
    EndpointTrackingMap *endpoints = nullptr;
    if (stat.statistic_type ==
...
RosGraphMonitor::statusMsg(uint8_t level, const std::string &message,
...
std::unique_ptr<rosgraph_monitor_msgs::msg::RosGraph>
RosGraphMonitor::generate_rosgraph() {
  ...
  // TODO(troy): Add rest of the node graph info here, like node namespaces,
  // publishers, subscriptions, etc.
  for (const auto &[node_name, node_info] : nodes_) {
    if (ignore_node(node_name)) {
      continue;
    }
