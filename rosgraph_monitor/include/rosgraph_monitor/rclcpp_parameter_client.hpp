...
///
/// A thin adapter over rclcpp::AsyncParametersClient.
/// Every call uses the callback form,
/// so responses arrive on the executor and no thread is spent waiting on a node that may never answer.
/// One client is kept per observed node and dropped when the node goes away.
...
private:
  ...
  using ClientMap = std::unordered_map<std::string, std::shared_ptr<rclcpp::AsyncParametersClient>>;
  MutexProtected<ClientMap> clients_;
};
