...
public:
  ...
  MOCK_METHOD((std::map<std::string, std::vector<std::string>>),
  ...
  MOCK_METHOD((std::map<std::string, std::vector<std::string>>),
              ...
              (const std::string &, const std::string &, bool),
              (const, override));
  MOCK_METHOD((std::vector<std::string>), get_node_names, (),
              (const, override));
  MOCK_METHOD((std::vector<std::tuple<std::string, std::string, std::string>>),
  ...
  MOCK_METHOD(size_t, count_publishers, (const std::string &),
              (const, override));
  MOCK_METHOD(size_t, count_subscribers, (const std::string &),
              (const, override));
  MOCK_METHOD(const rcl_guard_condition_t *, get_graph_guard_condition, (),
              (const, override));
#ifndef ROS2_HUMBLE
  ...
  void notify_shutdown() override { cv_.notify_all(); }
  ...
  MOCK_METHOD((std::vector<rclcpp::TopicEndpointInfo>),
              get_publishers_info_by_topic, (const std::string &, bool),
              (const, override));
  MOCK_METHOD((std::vector<rclcpp::TopicEndpointInfo>),
              get_subscriptions_info_by_topic, (const std::string &, bool),
              (const, override));
...
static rclcpp::TopicEndpointInfo blank_info() {
  ...
  // It's okay to use this local pointer only because the data is copied into
  // std::strings and the pointers do not live on past this function
  const char *tmp = "";
  cinfo.node_name = tmp;
...
struct Endpoint {
  ...
  Endpoint(rclcpp::EndpointType endpoint_type, const std::string &topic_name,
           const std::string &topic_type, const std::string &node_name,
    ...
    // Note this'll break past 255 endpoints, but let's don't build tests that
    // large
    next_gid[0]++;
  }
};
...
protected:
  GraphMonitorTest() : logger_(rclcpp::get_logger("test_graphmon")) {
    ...
    EXPECT_CALL(*node_graph_, count_publishers)
        .WillRepeatedly([this](const std::string &topic_name) {
          size_t count = 0;
          for (const auto &[gid, endpoint] : endpoints_) {
            if (endpoint.info.endpoint_type() ==
    ...
    graphmon_.emplace(
        node_graph_, [this]() { return now_; }, logger);
  }
  ...
  void set_node_names(std::vector<std::string> node_names) {
    // Add the root namespace / onto the names, which should not be specified
    // with it
    node_names_.clear();
  ...
  Endpoint add_pub(const std::string &topic_name, const std::string &topic_type,
    ...
    return add_endpoint(Endpoint(
        rclcpp::EndpointType::Publisher, topic_name, topic_type,
        node_name.value_or(default_node_name_), qos.value_or(default_qos_)));
  }

  Endpoint add_sub(const std::string &topic_name, const std::string &topic_type,
                   const std::optional<std::string> &node_name = std::nullopt,
                   const std::optional<rclcpp::QoS> &qos = std::nullopt) {
    return add_endpoint(Endpoint(
        rclcpp::EndpointType::Subscription, topic_name, topic_type,
        node_name.value_or(default_node_name_), qos.value_or(default_qos_)));
  }
  ...
  rosgraph_monitor_msgs::msg::TopicStatistic
  make_stat(uint8_t statistic_type, std::chrono::milliseconds mean,
            const std::optional<std::string> &node_name = std::nullopt,
  ...
  struct StatusCheck {
    ...
    StatusCheck(uint8_t level,
                std::optional<std::string> name_suffix = std::nullopt)
        : level(level), name_suffix(name_suffix) {}
  };

  void check_statuses(std::vector<StatusCheck> expectations,
                      std::string testname) {
    ...
    for (size_t i = 0; i < expectations.size(); i++) {
      ...
      EXPECT_EQ(actual.level, static_cast<uint8_t>(expect.level))
          << repr << testname;
      if (expect.name_suffix) {
        EXPECT_TRUE(ends_with(actual.name, *expect.name_suffix))
            << repr << testname;
      }
    }
  }
...
TEST_F(GraphMonitorTest, endpoint_continuity_ignored_subbernode_sub) {
  ...
  check_statuses(
      {WARN},
      "Regular's pub is discontinuous since subscriber's node ignored.");
}
...
TEST_F(GraphMonitorTest, endpoint_continuity_ignore_topic_types) {
  ...
  graphmon_->config().continuity.ignore_topic_types = {ignore_type1,
                                                       ignore_type2};
  set_node_names({default_node_name_});
...
TEST_F(GraphMonitorTest, endpoint_continuity_ignore_topic_names) {
  ...
  graphmon_->config().continuity.ignore_topic_names = {ignore_topic1,
                                                       ignore_topic2};
  set_node_names({default_node_name_});
...
TEST_F(GraphMonitorTest, topic_frequency_no_deadline_dont_care) {
  ...
  check_statuses({},
                 "Topic with no deadline reports nothing based on topic stats");
}
...
TEST_F(GraphMonitorTest, topic_frequency_happy) {
  ...
  stats.statistics.push_back(
      make_stat(rosgraph_monitor_msgs::msg::TopicStatistic::PUBLISHED_PERIOD,
                std::chrono::milliseconds(9)));
  stats.statistics.push_back(
      make_stat(rosgraph_monitor_msgs::msg::TopicStatistic::RECEIVED_PERIOD,
...
TEST_F(GraphMonitorTest, topic_frequency_slow) {
  ...
  graphmon_->on_topic_statistics(stats);
  check_statuses({WARN, WARN},
                 "Topic with deadline sending and receiving too slow");
}
...
TEST_F(GraphMonitorTest, topic_frequency_fast) {
  ...
  stats.statistics.push_back(
      make_stat(rosgraph_monitor_msgs::msg::TopicStatistic::RECEIVED_PERIOD,
                std::chrono::milliseconds(8)));
  graphmon_->on_topic_statistics(stats);
  check_statuses({StatusCheck(OK, "PublishFrequency::/topic1"),
                  StatusCheck(OK, "ReceiveFrequency::/topic1")},
...
TEST_F(GraphMonitorTest, topic_frequency_tx_bad_rx_good) {
  ...
  check_statuses({StatusCheck(OK, "PublishFrequency::/topic1"),
                  StatusCheck(OK, "ReceiveFrequency::/topic1")},
                 "Confusing case! Topic with deadline sending too fast but "
                 "receiving at correct");
}
...
TEST_F(GraphMonitorTest, topic_frequency_not_received) {
  ...
  check_statuses({},
                 "Discovered topics 1 second ago, should not be stale yet.");

  now_ = rclcpp::Time(104, 0, RCL_ROS_TIME);
  check_statuses({ERROR, ERROR},
                 "Enough time has passed to call topic stats stale");
}
